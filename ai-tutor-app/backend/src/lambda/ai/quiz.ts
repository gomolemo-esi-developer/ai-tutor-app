...
export async function handleGenerateQuiz(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    return LambdaUtil.wrap(async () => {
        ...
        let ragResponse: any;
        try {
            ragResponse = await ragService.generateQuiz(
                module.moduleId,
            ...
            LoggerUtil.error('[RAG] Quiz generation failed', {
                error: ragError instanceof Error ? ragError.message : String(ragError),
                moduleId: module.moduleId,
                documentCount: ragDocumentIds.length
            });
            throw new Error(`Failed to generate quiz from RAG service: ${ragError instanceof Error ? ragError.message : String(ragError)}`);
        }
