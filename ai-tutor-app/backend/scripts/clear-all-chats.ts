 ...
 * 
 * Usage: npx ts-node scripts/clear-all-chats.ts
 * 
 * WARNING: This will delete all chat data. Use with caution!
 */
...
async function main() {
  ...
  try {
    ...
    console.log(`   Will delete: ${chatSessions.length} chat sessions`);
    console.log(`   Will delete: ${chatMessages.length} chat messages`);
    console.log(
      '\n   To proceed, set environment variable: CONFIRM_DELETE=true'
    );
    console.log('   Example: CONFIRM_DELETE=true npx ts-node scripts/clear-all-chats.ts\n');
