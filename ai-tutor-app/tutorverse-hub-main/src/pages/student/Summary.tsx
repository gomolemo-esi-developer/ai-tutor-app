...
const parseSummaryContent = (summaryText: string): SummaryData => {
    ...
    return {
        ...
        topics: ['Fundamentals', 'Implementation', 'Best Practices', 'Case Studies'],
    };
};
...
const Summary: React.FC = () => {
    ...
    const generateSummary = async (items: ContentItem[]) => {
        try {
            ...
            if (summaryData) {
                setSummary(parseSummaryContent(summaryData));
            } else {
                // Fallback to parsed default
                setSummary(parseSummaryContent('Summary generated from selected content.'));
            }
        } catch (err) {
            console.error('Failed to generate summary:', err);
            toast.error('Failed to generate summary, using default structure');
            // Provide default structure on error
            setSummary(parseSummaryContent('Unable to generate summary at this time. Please try again later.'));
        } finally {
            setIsLoading(false);
        }
    };
                            ...
                            <Card className="bg-card border-border card-elevated">
                                <CardContent className="p-3 md:p-4 flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm md:text-base font-medium">{summary.difficulty}</p>
                                    </div>
                                </CardContent>
                            </Card>
                            <Card className="bg-card border-border card-elevated">
